/** Edit History:
	...
	- Changed [writeBlock(gMotionModal.format(1), "X0", "Y0", "Z15"); // Return to zero.] Was previously "Z0" and hitting origin
      ...
      (v10) Martindb: Added waterjet/plasma/laser cutter support (Enable waterjet/laser/plasma support in preferences=>preview)
**/
/**
...
description = "MPCNC posts processor";
vendor = "Autodesk, Inc.";
...
properties = {
  ...
  enableM106SpinsdleControl: false, // Enable M106/M107 for spindle speed control
  laserOn: "M106 S255", // Default gcode for fire plasma/laser
  laserOff: "M107" // Default gcode to turn off plasma/laser
};
...
//      onCommand(COMMAND_OPTIONAL_STOP);
  ...
  if (insertToolCall ||
    ...
    if (tool.spindleRPM < 1 && !tool.jetTool) {
...
function setDeviceMode(enable) {
  if (enable != deviceOn) {
    deviceOn = enable;
    if (enable) {
      writeBlock(properties.laserOn);
    } else {
      writeBlock(properties.laserOff);
    }
  }
}
