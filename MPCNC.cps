...
properties = {
  graphite: false, // graphite if true and steel otherwise
  RMAX: spatial(2, MM), // RMAX value in millimeters only for finishing strategies
  cleanAir: false, // clean tool using air
...
writeBlock("*/")
  ...
  if (getCurrentPosition().z <= initialPosition.z) {
	...
	writeComment("Start Position")
    writeBlock("G0", zOutput.format(initialPosition.z), xOutput.format(initialPosition.x), yOutput.format(initialPosition.y) );
  } else {
    //writeBlock("G0",
    //  gAbsIncModal.format(90),
    //  "G0", xOutput.format(initialPosition.x), yOutput.format(initialPosition.y)
    //);
  ...
  if ((cycleType == "tapping") || (cycleType == "left-tapping") || (cycleType == "right-tapping")) {
    ...
    if (_x || _y) {
      if (mapRapidToHighFeed) {		  
        writeBlock("G1", _x, _y, feedOutput.format(highFeedrate));
      } else {		  
        writeBlock("G0", _x, _y);
        feedOutput.reset();
      }
    ...
    if (mapRapidToHighFeed) {
      writeBlock("G1", zOutput.format(cycle.retract), feedOutput.format(highFeedrate));
    } else {
      writeBlock("G0", zOutput.format(cycle.retract));
      feedOutput.reset();
    }
    ...
    zOutput.reset();
    if (mapRapidToHighFeed) {
      writeBlock("G1", zOutput.format(cycle.clearance), feedOutput.format(highFeedrate));
    } else {
      writeBlock("G0", zOutput.format(cycle.clearance));
      feedOutput.reset();
    }
  } else {
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    if (mapRapidToHighFeed) {
      writeBlock("G1", x, y, z, feedOutput.format(highFeedrate));
    } else {
      writeBlock("G0", x, y, z);
      feedOutput.reset();
    }
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      case RADIUS_COMPENSATION_LEFT:
        writeBlock("G1", gFormat.format(41), x, y, z, f);
        break;
      case RADIUS_COMPENSATION_RIGHT:
        writeBlock("G1", gFormat.format(42), x, y, z, f);
        break;
      default:
        writeBlock("G1", gFormat.format(40), x, y, z, f);
      }
    } else {
		//writeComment("write position onLinear")
	  //writeBlock("G0", zOutput.format(_z), xOutput.format(_x), yOutput.format(_y) );
...
function onCommand(command) {
	writeComment("write position command")
  switch (command) {
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    ...
    // writeBlock(gAbsIncModal.format(90), "G0", homeX, homeY);
  }
